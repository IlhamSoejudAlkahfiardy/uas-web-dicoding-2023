...
const createBook = (request, h) => {
  ...
  if (year <= 0) {
    const response = h.response({
      status: "fail",
      message: "Gagal menambahkan buku. Masukkan tahun dengan benar!",
    });
    response.code(400);
    return response;
  }
  ...
  if (readPage < 0 || pageCount <= 0) {
    const response = h.response({
      status: "fail",
      message:
        "Gagal menambahkan buku. Masukkan readPage atau padeCount dengan benar!",
    });
    response.code(400);
    return response;
  }
